...
def sendMail(body,currentTime):
    ...
    msg['To'] = ','.join(to_mail)
    msg['Subject'] = Header('<%s>:南区运行方案'%currentTime, 'utf-8').encode()
    # plain代表纯文本
...
def main():
    ...
    # 供回水温度映射表
    data = pd.read_excel('供回水温度映射表.xlsx',sheetname='南区供回水温度映射表')
    Supply2BackMap = {}
    ...
    SQL_2 = 'SELECT ZG_HS_T FROM bd_sq_mainstation ORDER BY create_time DESC LIMIT 5'
    conn.cursor.execute(SQL_2)
    HST = np.mean([float(i[0]) for i in conn.cursor.fetchall()])
    
    SQL_2 = 'SELECT ZG_GS_T FROM bd_sq_mainstation ORDER BY create_time DESC LIMIT 5'
    conn.cursor.execute(SQL_2)
    GST = np.mean([float(i[0]) for i in conn.cursor.fetchall()])
    # 获取锅炉能正常开启的台数
    SQL_3 = 'SELECT GL1_O_PR,GL2_O_PR,GL3_O_PR,GL4_O_PR,GL5_O_PR,GL6_O_PR,GL7_O_PR,GL8_O_PR FROM bd_sq_mainstation ORDER BY create_time DESC LIMIT 20'
    conn.cursor.execute(SQL_3)
    O_PR_Num_1 = np.array(conn.cursor.fetchall())
    O_PR_Num_1 = [1 if i >= 1 else 0 for i in np.mean(O_PR_Num_1,axis=0)]
    O_PR_Num_1 = sum(O_PR_Num_1)
    SQL_3 = 'SELECT GL7_O_PR,GL8_O_PR FROM bd_nq_mainstation_gl ORDER BY create_time DESC LIMIT 20'
    conn.cursor.execute(SQL_3)
    O_PR_Num_2 = np.array(conn.cursor.fetchall())
    O_PR_Num_2 = [1 if i >= 1 else 0 for i in np.mean(O_PR_Num_2,axis=0)]
    O_PR_Num_2 = sum(O_PR_Num_2)
    O_PR_Num = O_PR_Num_1 + O_PR_Num_2
    ...
    for t in np.arange(26,59,0.5):
        ...
        if d < minDiff:
            ...
            minDiff = d
    '''        
    targetTDiff = max(0.8,abs(targetT - GST)*0.5)
...
#        targetT -= 6
    ...
    P_SZ = upTemp * P
    P_SZ_Sin = P_SZ/NUM
    P_SZ_Sin = min(int(P_SZ_Sin - 16.4),92)
    P_SZ_Sin = min(max(P_SZ_Sin,6),98)
    ...
    try:
        postCommend('100005',1,tempData,Loading/10,int(P_SZ_Sin),targetT,upTemp,currentTime_)
    except:
        print('WeChat Error')
        pass
    
    BoilerCode = {1:1,2:2,3:7,4:8,5:5,6:6,7:7,8:8}
    ...
    for i in range(2):
        ...
        if i in [2,3,7,8]:
            data = {
                "itemName":"SQ\GL\GL%d_O_PR_K.PV"%i,
    ...
    SQL = SQL%('南区',currentTime_,P_SZ_Sin,CS)
    conn.cursor.execute(SQL)
    conn.closeConnector()
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if current.minute >= 0 and current.minute < 8:
            main()
            time.sleep(60*20)
        elif current.minute >= 30 and current.minute < 38:
            main()
            time.sleep(60*20)
        else:
            time.sleep(20)
